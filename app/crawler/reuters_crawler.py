...
class ReutersCrawler:
    ...
    def __init__(self):
        ...
        self.news_list_url = f"{self.base_url}/markets"
        self.headers = {
    ...
    async def crawl(self, max_news: int = 50) -> List[CrawledNews]:
        """Reuters 뉴스 크롤링"""
        log.info(f"Reuters 뉴스 크롤링 시작 (최대 {max_news}개)")
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                news_items = soup.select('[data-testid="MediaStoryCard"], article')
                ...
                log.info(f"Reuters 뉴스 크롤링 완료: {len(crawled_news)}개")
                return crawled_news
                
        except Exception as e:
            log.error(f"Reuters 뉴스 크롤링 실패: {e}")
            return []
    
    async def _parse_news_item(self, item, client: httpx.AsyncClient) -> CrawledNews:
        """개별 뉴스 아이템 파싱"""
        title_tag = item.select_one('a[data-testid="Heading"], h3 a, a')
        if not title_tag:
            return None
    ...
    async def _fetch_news_content(self, url: str, client: httpx.AsyncClient) -> tuple[str, str]:
        ...
        try:
            ...
            soup = BeautifulSoup(response.text, 'lxml')
            content_tag = soup.select_one('[data-testid="paragraph-0"], .article-body, article p')
            content = content_tag.get_text(strip=True) if content_tag else "본문 없음"
            
            image_tag = soup.select_one('article img, .article-media img')
            image_url = image_tag['src'] if image_tag and 'src' in image_tag.attrs else None
            
            return content[:5000], image_url
        except Exception as e:
            log.error(f"Reuters 본문 가져오기 실패: {e}")
            return "본문 없음", None
